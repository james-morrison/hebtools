# -*- coding: utf-8 -*-
"""
Created on Thu Aug 16 10:24:38 2012

Adapted from https://gist.github.com/1178136, a pandas data frame is loaded,
the heave timeseries is passed to the peakdetect function which returns the 
local maxima and minima, the position of those maxima and minima are returned 
to the original DataFrame within an extra column called 'extrema', 1 
representing a local maxima and -1 a minima and NaN for neither, the peakdetect
 algorithm needs tweaking for unusual situations
...
def peakdetect(y_axis, x_axis = None, lookahead = 2, delta=0):
    """
...
def get_extrema_timestamps(extrema):
...
if __name__ == "__main__":
    ...
    raw_disp = pd.load('raw_buoy_displacement_pandas')
    y = raw_disp['heave']
    index = y.index
    _max, _min = peakdetect(y)
    maxima_timestamps = get_extrema_timestamps(_max)
    minima_timestamps = get_extrema_timestamps(_min)
    ...
    raw_disp_with_extrema = raw_disp.join(extrema_df)
